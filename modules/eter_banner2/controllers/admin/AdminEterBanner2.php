...
class AdminEterBanner2Controller extends ModuleAdminController
{
    /**
    ...
    */
    ...
    {
        parent::initContent();
        $this->uploadImageDir = _PS_MODULE_DIR_.'eter_banner2/images/';
    ...
    */
    ...
    /**
    * Process after validetForm(), save img name and copy image
    */
    public function postFormProcess() {
        $res = true;
        if($image = $this->module->uploadImage('image',$this->uploadImageDir)) {
            $res &= Configuration::updateValue('BANNER2_IMAGE_NAME', $image);
        } 
        if($image_mobile = $this->module->uploadImage('image_mobile',$this->uploadImageDir)) {
            $res &= Configuration::updateValue('BANNER2_IMAGE_NAME_MOBILE', $image_mobile);
        } 
        $languages = Language::getLanguages(false);
        foreach ($languages as $lang) {
            $res &= Configuration::updateValue('BANNER2_URL_L'.$lang['id_lang'], Tools::getValue('url_'.$lang['id_lang']));
        }
        return $res;
    }
    /**
    ...
    */
    public function renderForm()
    {
        ...
        foreach ($languages as $lang) {
            $lang = $lang['id_lang'];
            $data['url'][$lang] = Configuration::get('BANNER2_URL_L'.$lang);
        }
        if($image = Configuration::get('BANNER2_IMAGE_NAME')){
        ...
        if($image_mobile =  Configuration::get('BANNER2_IMAGE_NAME_MOBILE')){
        ...
        $helper->back_url =  $this->context->link->getAdminLink('AdminEterBanner2', false);
        $helper->currentIndex = $this->context->link->getAdminLink('AdminEterBanner2', false);
        $helper->token = Tools::getAdminTokenLite('AdminEterBanner2');
