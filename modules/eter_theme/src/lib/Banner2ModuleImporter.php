...
class Banner2ModuleImporter
{
	/**
     * Main method
     */
	public function run($theme) {
        $jsonData = file_get_contents(_ETER_IMPORT_DIR_.$theme.'/eter_modules/banner2/banner2.json');
        $data = json_decode($jsonData, true);
		$this->install($data,$theme);
        return ['success' => true];
	}
     ...
     * Install new banner1
     */
	public function install($data,$theme) {
        $image = $data['image'];
        $imageMobile = $data['image_mobile'];
        $from = _ETER_IMPORT_DIR_.$theme.'/eter_modules/banner2/'.$image;
        $to = _PS_MODULE_DIR_.'eter_banner2/images/'.$image;
        if(@copy($from,$to) && isset($image)){
            Configuration::updateValue('BANNER2_IMAGE_NAME', $image);
        }

        $from = _ETER_IMPORT_DIR_.$theme.'/eter_modules/banner2/'.$imageMobile;
        $to = _PS_MODULE_DIR_.'eter_banner2/images/'.$imageMobile;
        if(@copy($from,$to) && isset($imageMobile)){
            Configuration::updateValue('BANNER2_IMAGE_NAME_MOBILE', $imageMobile);
        }
        
        $languages = Language::getLanguages(false);
        foreach ($languages as $lang) {
            Configuration::updateValue('BANNER2_URL_L'.$lang['id_lang'], $data['url_target']);
        }
    }
}