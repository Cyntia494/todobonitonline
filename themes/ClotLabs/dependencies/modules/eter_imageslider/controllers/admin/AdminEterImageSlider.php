...
class AdminEterImageSliderController extends ModuleAdminController
{
    ...
    */
    public function validateForm() 
    {
        ...
        if($error = $this->module->validateImage('background_mobile',$this->path,500,700,!$id_homeslider_slides)) {
            $res &= false;
            $this->errors[] = $error;
        }
        ...
        foreach ($languages as $lang) {
            ...
            if (Tools::getValue('display_mode') == "animated") {
    ...
    */
    public function postFormProcess() {
        ...
        $slide->display_mode = Tools::getValue('display_mode');
        ...
        if($background = $this->module->uploadImage('background',$this->path,"hero-{$slide->id}")) {
            $slide->background = $background;
        }
        if($background_mobile = $this->module->uploadImage('background_mobile',$this->path,"hero-mobile-{$slide->id}")) {
            $slide->background_mobile = $background_mobile;
        }
        if($image = $this->module->uploadImage('image',$this->path,"animatedimage-{$slide->id}")) {
            $slide->image = $image;
        }
        return $slide->save();
    }
    ...
    public function renderAddForm()
    {
        ...
        if (Tools::isSubmit('id_homeslider_slides')) {
            ...
            if($data->background_mobile){
                $src = $this->context->link->getMediaLink('/modules/eter_imageslider/images/'.$data->background_mobile);
                $backhtmlmobile = '<img alt="preview" style="height:100px;" src="'.$src.'">';
            }
        ...
        $modes[] = ['mode' => "animated", 'name' => 'Animated'];
        $modes[] = ['mode' => "image_only", 'name' => 'Image Only'];
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'file',
                        'label' => $this->l('Background Image Desktop'),
                        'name' => 'background',
                        'required' => true,
                        'display_image' => true,
                    ...
                    array(
                        'type' => 'file',
                        'label' => $this->l('Background Image Mobile'),
                        'name' => 'background_mobile',
                        'required' => true,
                        'display_image' => true,
                        'image' => $backhtmlmobile ? $backhtmlmobile : false,
                        'desc' => $this->l('Maximum image size: 500 x 700')
                    ),
                    array(
                        'type' => 'file',
                        ...
                        'form_group_class' => 'depends_mode hidden',
                    ),
                    array(
                        'type' => 'select',                              
        ...
        $js =  '<script type="text/javascript">
            window.toggleInputs = function(mode) {
                console.log(mode);
                if (mode == "animated") {
                    $(".depends_mode").removeClass("hidden");
                } else {
                    $(".depends_mode").addClass("hidden");
                }
            }
            toggleInputs($(\'select[name="display_mode"]\').val());
            $(\'select[name="display_mode"]\').change(function () {
                toggleInputs($(this).val());
            });
        </script>';
        return $helper->generateForm(array($fields_form)).$js;
    }
