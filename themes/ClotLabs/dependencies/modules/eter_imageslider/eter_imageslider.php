...
class Eter_ImageSlider extends Module implements WidgetInterface
{
    ...
    public function createTables()
    {
        ...
        ');
        $res &= EterHelper::addColumn("eter_homeslider_slides", "background_mobile", "varchar(255)");
        $res &= EterHelper::addColumn("eter_homeslider_slides", "display_mode", "varchar(255)");
        return $res;
    }
    /**
     * deletes tables
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        foreach ($data as &$value) {
            ...
            $value['image'] = $src;
            $src = $this->context->link->getMediaLink('/modules/eter_imageslider/images/'.$value['background_mobile']);
            $value['background_mobile'] = $src;
            if(Context::getContext()->isMobile()){
                $value['backimage'] = $value['background_mobile'];
            } else {
                $value['backimage'] = $value['background'];
            }
        }
        return [
            'homeslider' => [
