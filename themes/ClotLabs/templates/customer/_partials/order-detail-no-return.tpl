...
{block name='order_products_table'}
  <div class="box hidden-sm-down order-products">
    <h3>{l s='Order Products' d='Shop.Theme.Customeraccount'}</h3>
      ...
      <div class="order-item">
        <div class="row">
          ...
          <div class="col-sm-7 qty">
            <div class="row">
              <div class="col-xs-12 col-sm-4 text-sm-left text-xs-left">
                {$product.price}
              </div>
              <div class="col-xs-12 col-sm-4">
                {if $product.customizations}
                  ...
                  {/foreach}
                {else}
                  <span class="hidden-md-up">{l s='Qty:' d='Shop.Theme.Catalog'}</span>{$product.quantity}
                {/if}
              </div>
    ...
    {foreach $order.subtotals as $line}
      {if $line.value}
        <div class="order-total row">
          <div class="col-xs-12 col-sm-8"><strong>{$line.label}</strong></div>
          <div class="col-xs-12 col-sm-4 text-xs-left text-sm-right">{$line.value}</div>
        </div>
      {/if}
    {/foreach}
    <div class="order-total row">
      <div class="col-xs-12 col-sm-8"><strong>{$order.totals.total.label}</strong></div>
      <div class="col-xs-12 col-sm-4 text-xs-left text-sm-right">{$order.totals.total.value}</div>
    </div>
  </div>
{/block}
