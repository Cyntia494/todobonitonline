          ...
          <tr>
            <td>
              {if !$product.customizations}
                ...
                <input type="checkbox" id="cb_{$product.id_order_detail}" name="ids_order_detail[{$product.id_order_detail}]" value="{$product.id_order_detail}" data-id_order_detail="{$product.id_order_detail}" data-id_customization="0">
              </span>
              {else}
                {foreach $product.customizations  as $customization}
                  <span id="_desktop_product_customization_line_{$product.id_order_detail}_{$customization.id_customization}">
                  <input type="checkbox" id="cb_{$product.id_order_detail}" name="customization_ids[{$product.id_order_detail}][]" value="{$customization.id_customization}" data-id_order_detail="{$product.id_order_detail}" data-id_customization="{$customization.id_customization}">
                </span>
                {/foreach}
              {/if}
            </td>
            ...
            <td class="qty">
              {if !$product.customizations}
                <div class="current">
                  <p><strong>{l s='Qty' d='Shop.Theme.Customeraccount'}</strong>{$product.quantity}</p>
                  <p><strong>{l s='Returned' d='Shop.Theme.Customeraccount'}</strong>:{$product.qty_returned}</p>
                </div>
                {if $product.quantity > $product.qty_returned}
                  <div class="select" style="display: none;" id="_desktop_return_qty_{$product.id_order_detail}">
              ...
              {else}
                {foreach $product.customizations as $customization}
                  ...
                  <div class="select" style="display: none;" id="_desktop_return_qty_{$product.id_order_detail}_{$customization.id_customization}">
                    <select
            ...
            <div class="content">
              <div class="row">
                ...
                <div class="col-sm-7 qty">
                  <div class="row">
                    <div class="col-xs-12 text-sm-left text-xs-left">
                      {$product.price}
                    </div>
                    <div class="col-xs-12">
                      {if $product.customizations}
      ...
      {foreach $order.subtotals as $line}
        {if $line.value}
          <div class="order-total row">
            <div class="col-xs-6"><strong>{$line.label}</strong></div>
            <div class="col-xs-6 text-xs-right">{$line.value}</div>
          </div>
        {/if}
      {/foreach}
      <div class="order-total row">
        <div class="col-xs-6"><strong>{$order.totals.total.label}</strong></div>
        <div class="col-xs-6 text-xs-right">{$order.totals.total.value}</div>
      </div>
    </div>
